...
let max =arr[str[0].charCodeAt(0)];
let ch;
//here we find max occurence to checj value all index,which will be greater,will be max
for (let i = 0; i < str.length; i++) {
      
        if (max<arr[str[i].charCodeAt(0)]) {
            max=arr[str[i].charCodeAt(0)];
...
let str ="Welcome to coding world";
 console.log(maxfrequent(str));